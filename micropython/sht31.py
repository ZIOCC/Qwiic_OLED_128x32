...
# This is an updated library from the SHT30 libirary, we modified the address Pins to str instead of int
# so it can support different pyboard I2C ports
__version__ = '0.2.3'
__author__ = 'Roberto Sánchez'
__license__ = "Apache License 2.0. https://www.apache.org/licenses/LICENSE-2.0"

# Zio QWiic SHT31's default I2C address is 0x44
...
class SHT31:
    ...
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
    ...
    def __init__(self, scl_pin="Y9", sda_pin="Y10", delta_temp=0, delta_hum=0, i2c_address=DEFAULT_I2C_ADDRESS):
        self.i2c = I2C(scl=Pin(scl_pin), sda=Pin(sda_pin))
        self.i2c_addr = i2c_address
        self.set_delta(delta_temp, delta_hum)
        time.sleep_ms(50)

    def init(self, scl_pin="Y9", sda_pin="Y10"):
        """
        ...
        try:
            ...
            for i in range(response_size//3):
                if not self._check_crc(data[i*3:(i+1)*3]):  # pos 2 and 5 are CRC
                    raise SHT31Error(SHT31Error.CRC_ERROR)
            if data == bytearray(response_size):
                raise SHT31Error(SHT31Error.DATA_ERROR)
            return data
        except OSError as ex:
            if 'I2C' in ex.args[0]:
                raise SHT31Error(SHT31Error.BUS_ERROR)
            raise ex
    ...
    def clear_status(self):
        ...
        """
        return self.send_cmd(SHT31.CLEAR_STATUS_CMD, None)

    def reset(self):
        """
        Send a soft-reset to the sensor
        """
        return self.send_cmd(SHT31.RESET_CMD, None)
    ...
    def status(self, raw=False):
        ...
        """
        data = self.send_cmd(SHT31.STATUS_CMD, 3, read_delay_ms=20)
    ...
    def measure(self, raw=False):
        ...
        """
        data = self.send_cmd(SHT31.MEASURE_CMD, 6)
...
class SHT31Error(Exception):
    ...
    def get_message(self):
        if self.error_code == SHT31Error.BUS_ERROR:
            return "Bus error"
        elif self.error_code == SHT31Error.DATA_ERROR:
            return "Data error"
        elif self.error_code == SHT31Error.CRC_ERROR:
            return "CRC error"
        else:
